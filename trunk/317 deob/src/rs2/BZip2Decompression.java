...
public final class BZip2Decompression {
	...
	private static void readNextFile(ArchiveInfo archiveInfo) {
		int tMinLength = 0;
		int limit[] = null;
		int tBase[] = null;
		int tPerm[] = null;
		archiveInfo.anInt578 = 1;
		if (ArchiveInfo.buffer == null) {
			ArchiveInfo.buffer = new int[archiveInfo.anInt578 * 0x186a0];
		}
		...
		while (isReading) {
			...
			for (int perm = tPerm[bitsReadValue - tBase[minLength]]; perm != endOfBlockChar;) {
				if (perm == 0 || perm == 1) {
					...
					for (; index > 0; index--) {
						ArchiveInfo.buffer[i6] = byte5 & 0xff;
						i6++;
					}
				} else {
					int j11 = perm - 1;
					...
					ArchiveInfo.buffer[i6] = archiveInfo.usedBitmap[byte6 & 0xff] & 0xff;
					i6++;
					if (groupPosition == 0) {
						groupNumber++;
						groupPosition = 50;
			...
			for (int index = 0; index < i6; index++) {
				byte byte7 = (byte) (ArchiveInfo.buffer[index] & 0xff);
				ArchiveInfo.buffer[archiveInfo.anIntArray585[byte7 & 0xff]] |= index << 8;
				archiveInfo.anIntArray585[byte7 & 0xff]++;
			}

			archiveInfo.anInt581 = ArchiveInfo.buffer[archiveInfo.startingPointer] >> 8;
			archiveInfo.anInt584 = 0;
			archiveInfo.anInt581 = ArchiveInfo.buffer[archiveInfo.anInt581];
	...
	private static byte getByte(ArchiveInfo archiveInfo) {
		return (byte) getBits(8, archiveInfo);
	}

	private static byte getBit(ArchiveInfo archiveInfo) {
		return (byte) getBits(1, archiveInfo);
	}

	private static int getBits(int count, ArchiveInfo archiveInfo) {
		int j;
		do {
	...
	private static void createBitmaps(ArchiveInfo archiveInfo) {
	...
	private static final ArchiveInfo archiveInfo = new ArchiveInfo();
