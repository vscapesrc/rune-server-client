...
public final class JagexArchive {
	...
	public JagexArchive(byte data[]) {
		...
		if (j != decompressedSize) {
			...
			BZip2Decompression.resetAndRead(abyte1, decompressedSize, data, j, 6);
			finalBuffer = abyte1;
		...
		for (int index = 0; index < totalFiles; index++)
			if (identifiers[index] == identifier) {
				...
				if (!compressedAsWhole) {
					BZip2Decompression.resetAndRead(dataBuffer, decompressedSizes[index], finalBuffer, compressedSizes[index], startOffsets[index]);
				} else {
