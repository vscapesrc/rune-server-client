...
public final class RSInterface {
	...
	public static void unpack(JagexArchive interfaceArchive, RSFont fonts[], JagexArchive mediaArchive) {
		...
		ByteBuffer buffer = new ByteBuffer(interfaceArchive.getData("data"));
		int i = -1;
		int totalInterfaces = buffer.getShort();
		...
		while (buffer.offset < buffer.buffer.length) {
			...
			if (requiredmentIndex > 0) {
				...
				for (int index = 0; index < requiredmentIndex; index++) {
					rsi.valueCompareType[index] = buffer.getUByte();
					rsi.requiredValues[index] = buffer.getShort();
				}
			}
			int valueType = buffer.getUByte();
			if (valueType > 0) {
				...
				for (int valueIndex = 0; valueIndex < valueType; valueIndex++) {
					...
					for (int nextIndex = 0; nextIndex < size; nextIndex++) {
						rsi.valueIndexArray[valueIndex][nextIndex] = buffer.getShort();
					}
				}
			}
			if (rsi.type == 0) {
				rsi.scrollMax = buffer.getShort();
				rsi.showInterface = buffer.getUByte() == 1;
				int totalChildren = buffer.getShort();
				...
				for (int index = 0; index < totalChildren; index++) {
					rsi.children[index] = buffer.getShort();
					rsi.childX[index] = buffer.getSignedShort();
					rsi.childY[index] = buffer.getSignedShort();
				}
			}
			if (rsi.type == 1) {
				buffer.getShort();
			...
			if (rsi.type == 2) {
				...
				rsi.itemsSwappable = buffer.getUByte() == 1;
				rsi.isInventoryInterface = buffer.getUByte() == 1;
				rsi.usableItemInterface = buffer.getUByte() == 1;
				rsi.deletesTargetSlot = buffer.getUByte() == 1;
				rsi.invSpritePadX = buffer.getUByte();
				rsi.invSpritePadY = buffer.getUByte();
				...
				for (int index = 0; index < 20; index++) {
					...
					if (dummy == 1) {
						rsi.spritesX[index] = buffer.getSignedShort();
						rsi.spritesY[index] = buffer.getSignedShort();
			...
			if (rsi.type == 3) {
				rsi.filled = buffer.getUByte() == 1;
			}
			if (rsi.type == 4 || rsi.type == 1) {
				rsi.centered = buffer.getUByte() == 1;
				int fontId = buffer.getUByte();
				if (fonts != null) {
					rsi.font = fonts[fontId];
				}
				rsi.shadowed = buffer.getUByte() == 1;
			}
			if (rsi.type == 4) {
			...
			if (rsi.type == 6) {
				...
				if (value != 0) {
					...
					rsi.disabledMediaId = (value - 1 << 8) + buffer.getUByte();
				}
				value = buffer.getUByte();
				if (value != 0) {
					rsi.enabledMediaType = 1;
					rsi.enabledMediaId = (value - 1 << 8) + buffer.getUByte();
				}
				value = buffer.getUByte();
				if (value != 0) {
					rsi.disabledAnimation = (value - 1 << 8) + buffer.getUByte();
				} else {
					rsi.disabledAnimation = -1;
				}
				value = buffer.getUByte();
				if (value != 0) {
					rsi.enabledAnimation = (value - 1 << 8) + buffer.getUByte();
				} else {
					rsi.enabledAnimation = -1;
				}
				rsi.zoom = buffer.getShort();
				rsi.rotationX = buffer.getShort();
				rsi.rotationY = buffer.getShort();
			}
			if (rsi.type == 7) {
				rsi.inventory = new int[rsi.width * rsi.height];
				rsi.inventoryAmount = new int[rsi.width * rsi.height];
				...
				rsi.shadowed = buffer.getUByte() == 1;
				rsi.disabledColor = buffer.getInt();
				rsi.invSpritePadX = buffer.getSignedShort();
				rsi.invSpritePadY = buffer.getSignedShort();
				rsi.isInventoryInterface = buffer.getUByte() == 1;
				rsi.actions = new String[5];
				for (int index = 0; index < 5; index++) {
					rsi.actions[index] = buffer.getString();
			...
			if (rsi.actionType == 2 || rsi.type == 2) {
				...
				rsi.spellUsableOn = buffer.getShort();
			}
			if (rsi.type == 8) {
				rsi.disabledText = buffer.getString();
	...
	private Model getModelByType(int modelType, int modelId) {
		...
		if (modelType == 2) {
			model = NPCDef.getDef(modelId).method160();
		}
		if (modelType == 3) {
			model = Client.myPlayer.getPlayerModel();
		}
		if (modelType == 4) {
			model = ItemDef.getDef(modelId).getInventoryModel(50);
		}
		if (modelType == 5) {
			model = null;
		}
	...
	public Model getAnimatedModel(int id, int k, boolean enabled) {
		...
		if (k == -1 && id == -1 && animatedModel.anIntArray1640 == null) {
			return animatedModel;
		}
		Model model = new Model(true, FrameHeader.method532(k) & FrameHeader.method532(id), false, animatedModel);
		if (k != -1 || id != -1) {
			model.method469();
		}
		if (k != -1) {
